...
let all_snowflake = ["snowflake.png", "snowflake1.png"]
...
function createSnowFlake(){
  ...
  // Ranomly generate a number for the size and position
  snowflake.style.top = scrolled-50 + 'px';
  snowflake.style.left = Math.floor(Math.random() * 100) + '%';
  const flake_size = (Math.floor(Math.random() * (200-100))+100) + 'px'
  const snowflake_type = all_snowflake[Math.floor(Math.random() * 2)]
